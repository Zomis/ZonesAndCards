...
//		game.addCard(minion( 4,    COMMON, 3, 6, "Water Elemental").on(HStoneDamageDealtEvent.class, freezeDamagedMinion(), thisCard()).card());
...
//		game.addCard(spell( 3,    COMMON, "Frost Nova").effect("<b>Freeze</b>").effect("all enemy minions").card());
//		game.addCard(spell( 2,    COMMON, "Frostbolt").effect("Deal 3 damage to a character and").effect("<b>Freeze</b>").effect("it").card());
...
//		game.addCard(spell( 3,    COMMON, "Mirror Entity").effect("<b>Secret:</b>").effect("When your opponent plays a minion, summon a copy of it").card());
		...
		game.addCard(spell( 6,      RARE, "Blizzard").effect(forEach(and(not(samePlayer()), allMinions()), null, combined(damage(2), giveAbility(HSAbility.FROZEN)))).card());
