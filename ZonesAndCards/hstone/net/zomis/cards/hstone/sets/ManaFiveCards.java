...
public class ManaFiveCards implements CardSet<HStoneGame> {
	...
	public void addCards(HStoneGame game) {
		...
		game.addCard(minion( 5,    COMMON, 4, 4, "Frostwolf Warlord").battlecry(forEach(and(allMinions(), not(thisCard())), selfPT(1, 1), null)).card());
...
//		game.addCard(minion( 5,      RARE, 4, 4, "Gadgetzan Auctioneer").effect("Whenever you cast a spell, draw a card").card());
		game.addCard(minion( 5,      RARE, 3, 5, "Stampeding Kodo").battlecry(toRandom(and(withAttackLess(2), not(samePlayer()), allMinions()), destroyTarget())).card());
