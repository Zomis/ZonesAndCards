...
//		game.addCard(minion( 3,      FREE, 2, 2, "Raid Leader").effect("Your other minions have +1 Attack").card());
		game.addCard(minion( 3,      FREE, 3, 1, "Wolfrider").charge().card());
		game.addCard(minion( 3,    COMMON, 1, 3, "Acolyte of Pain").on(HStoneDamageDealtEvent.class, drawCard(), HSFilters.thisCard()).card()); // TODO: DamageD!!!
...
//		game.addCard(minion( 3,    COMMON, 2, 3, "Flesheating Ghoul").effect("Whenever a minion dies, gain +1 Attack. '").card());
...
//		game.addCard(minion( 3,    COMMON, 3, 3, "Raging Worgen").effect("<b>Enrage:</b>").effect("<b>Windfury</b>").effect("and +1 Attack").card());
		...
		game.addCard(minion( 3,    COMMON, 3, 2, "Shattered Sun Cleric").battlecry(to(and(allMinions(), samePlayer()), otherPT(1, 1))).card());
		game.addCard(minion( 3,    COMMON, 1, 4, "Silverback Patriarch").taunt().card());
//		game.addCard(minion( 3,    COMMON, 2, 3, "Tauren Warrior").taunt().effect("<b>Enrage:</b>").effect("+3 Attack").card());
...
//		game.addCard(minion( 3,      RARE, 0, 3, "Alarm-o-Bot").effect("At the start of your turn, swap this minion with a random one in your hand").card());
		...
		game.addCard(minion( 3,      RARE, 2, 3, "Coldlight Seer").battlecry(forEach(and(not(thisCard()), minionIsMurloc()), null, otherPT(0, 2))).card());
...
//		game.addCard(minion( 3,      RARE, 3, 3, "Mind Control Tech").effect("<b>Battlecry:</b>").effect("If your opponent has 4 or more minions, take control of one at random").card());
		game.addCard(minion( 3,      RARE, 2, 2, "Questing Adventurer").on(HStoneCardPlayedEvent.class, selfPT(1, 1), and(samePlayer(), not(thisCard()))).card());
//		game.addCard(minion( 3,      EPIC, 4, 2, "Big Game Hunter").effect("<b>Battlecry:</b>").effect("Destroy a minion with an Attack of 7 or more").card());
		game.addCard(minion( 3,		 EPIC, 3, 3, "Blood Knight").battlecry(forEach(minionWithDivineShield(), selfPT(3, 3), removeShield())).card());
//		game.addCard(minion( 3,      EPIC, 3, 3, "Blood Knight").effect("<b>Battlecry:</b>").effect("All minions lose").effect("<b>Divine Shield</b>").effect("Gain +3/+3 for each Shield lost").card());
