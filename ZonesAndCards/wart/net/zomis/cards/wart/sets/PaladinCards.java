...
public class PaladinCards implements CardSet<HStoneGame> {
	...
	public void addCards(HStoneGame game) {
		...
		game.addCard(spell( 4,      FREE, "Hammer of Wrath").effect(to(all(), combined(drawCard(), damage(3)))).card());
		...
		game.addCard(spell( 4,    COMMON, "Consecration").effect(forEach(opponent(), null, damage(2))).card());
...
//		game.addCard(spell( 5,      RARE, "Blessed Champion").effect( "Double a minion's Attack").card());
		...
		game.addCard(spell( 6,      EPIC, "Avenging Wrath").effect(repeat(fixed(8), toRandom(opponent().and(canTakeDamage(1)), damage(1)))).card());
