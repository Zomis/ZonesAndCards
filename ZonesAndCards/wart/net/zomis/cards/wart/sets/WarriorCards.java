...
public class WarriorCards implements CardSet<HStoneGame> {
	...
	public void addCards(HStoneGame game) {
		...
		game.addCard(spell( 1,      FREE, "Execute").effect(to(opponent().and(isDamaged()).and(allMinions()), destroyTarget())).forClass(HStoneClass.WARRIOR).card());
		...
		game.addCard(spell( 2,    COMMON, "Cleave").effect(iff(f.countAtLeast(opponentMinions(), 2), toMultipleRandom(fixed(2), opponentMinions(), damage(2)))).forClass(HStoneClass.WARRIOR).card());
...
//		game.addCard(spell( 2,      RARE, "Commanding Shout").effect("Your minions can't be reduced below 1 Health this turn.  Draw a card").forClass(HStoneClass.WARRIOR).card()); // TODO: Use enchantment for "Commanding Shout"
	...
	public static final HStoneEffect deal4or6damageIfIHaveLessThan12Health = 
			ifElse((src, dst) -> src.getPlayer().getHealth() <= 12, damage(6), damage(4));
