...
public class PriestCards implements CardSet<HStoneGame> {
	...
	public void addCards(HStoneGame game) {
		...
		game.addCard(minion( 4,    COMMON, 0, 5, "Lightspawn").effect(enchantSelfWithAttackEqualsHealth()).card());
		game.addCard(minion( 6,    COMMON, 6, 6, "Temple Enforcer").battlecry(to(allMinions().and(samePlayer()), otherPT(0, 3))).card());
...
//		game.addCard(minion( 7, LEGENDARY, 7, 7, "Prophet Velen").effect("Double the damage and healing of your spells and Hero Power").card());
		...
		game.addCard(spell( 5,    COMMON, "Holy Nova").effect(combined(forEach(opponent(), null, damage(2)), forEach(samePlayer(), heal(2), null))).card());
		game.addCard(spell( 1,    COMMON, "Inner Fire").effect(to(allMinions(), setOtherAttackEqualsHealth())).card());
		...
		game.addCard(spell( 6,      RARE, "Holy Fire").effect(to(all(), combined(damage(5), healMyHero(5)))).card());
